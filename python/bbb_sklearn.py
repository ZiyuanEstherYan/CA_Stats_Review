...
import graphviz
from pyrsm import gains, gains_plot, lift, confusion, profit_max, ROME_max
...
# Logistic regression with strong L1 (Lasso) regularization
...
model_gains = {m: gains(eval_dat.query("training == 0"), "buyer_yes", 1, m, qnt=10) for m in models}
model_gains.keys()
model_gains["y_lasso"]

# calculate lift for all models
model_lift = {m: lift(eval_dat.query("training == 0"), "buyer_yes", 1, m, qnt=10) for m in models}
model_lift.keys()
model_lift["y_lasso"]

gains_plot(
    eval_dat.query("training == 0"), "buyer_yes", 1, list(models[0:4]), qnt=10,
)

gains_plot(
    eval_dat.query("training == 0"), "buyer_yes", 1, list(models[4:]), qnt=10,
)
...
# checking for overfitting RF
dct = {
    "Train": eval_dat.query("training == 1"),
    "Test": eval_dat.query("training == 0")
}
gains_plot(dct, "buyer_yes", 1, "y_rf", qnt=10)

# checking for overfitting XGBoost
gains_plot(dct, "buyer_yes", 1, "y_xgb", qnt=10)
